# unity-package-logger
## com.openmygame.logger
...
Для включения логгирования нужно прописать define: ```ENABLE_LOGGING```
...
| ```SetIsExtractStacktrace```              | Устанавливает будет ли формироваться стектрейс для сообщения (нужно например для нативных логов с стектрейсом, для Editor'a лучше не трогать либо поставить ```false```, потому что в Editor'е можно включить стандартный стектрейс от Unity); дефолтный параметр - ```false``` |
...
| ```RenderAs```                 | Устанавливает формат, в котором будет рендериться результирующее сообщение - обычный текст.                                                                                                                                                                                                                                                                                                                                             |
...
Для этого можно использовать методы с префиксом Tag.
Пример:

```csharp
Log.TagDebug("Tag", "Debug current time with tag: {Time}", DateTime.Now);
Log.TagWarning("Tag", "Warning current time with tag: {Time}", DateTime.Now);
Log.TagError("Tag", "Error current time with tag: {Time}", DateTime.Now);
Log.TagFatal("Tag", "Fatal current time with tag: {Time}", DateTime.Now);
```
...
```csharp
var logWithTag = new LogWithTag("Time"); //var logWithTag = Log.TagLogger("Tag);
...
var tagLogger = Log.TagLogger("Time");
tagLogger.Debug("Debug current time with tag: {Time}", DateTime.Now);
tagLogger.Warning("Warning current time with tag: {Time}", DateTime.Now);
tagLogger.Error("Error current time with tag: {Time}", DateTime.Now);
tagLogger.Fatal("Fatal current time with tag: {Time}", DateTime.Now);

Log.Debug("Debug complex object: {@Value}", new { Value = 5 });
Log.Exception(new Exception("Test exception"));
Log.TagException("Tag", new Exception("Test exception"), "Error description");
```
